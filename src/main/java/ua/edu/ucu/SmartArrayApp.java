...
public class SmartArrayApp {
    ...
    public static Integer[] ftrPositiveIntegersSortAndMultiplyByTwo(Integer[] integers) {
    ...
    public static String[] findDistinctStudentNamesFromSecondYearWithGPAgeFourAndOrderedBySurname(Student[] students) {

        SmartArray studentSmartArray = new BaseArray(students);
        MyPredicate pr = t -> ((Student) t).getGPA() >= 2 * 2 && ((Student) t).getYear() == 2;
        ...
        MyComparator cmp = (oA, oB) -> {
            ...
            int[] len = {nmA.length(), nmB.length(), Math.min(nmB.length(), nmA.length())};
        ...
        for (Object student : studentSmartArray.toArray()) {
            ...
            result[i] = ((Student) student).getSurname() + " " + ((Student) student).getName();
            i++;
        }
